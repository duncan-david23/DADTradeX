...
const StockPage = () => {
  ...
  const stockD = stockData.find(item => item.id === parseInt(id));
  let cPrice = parseFloat(stockD.price); 
  
//   let priceChange = parseFloat(crypto.price_change_24h).toFixed(2);
  let priceChangePercentage = parseFloat(stockD.change).toFixed(2);
  let totalVolume = new Intl.NumberFormat('en-US').format(stockD.volume);
  let marketCap = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'GHC' }).format(stockD.marketCap);
...
const handleQuantityValue = (e) => {
    ...
    if (newQuantity) {
        setPriceValue((parseFloat(newQuantity) * price).toFixed(2));
    } else {
        setPriceValue('');
    }
};
                      ...
                      <div className='flex justify-between items-center'>
                        ...
                        <div className='flex gap-[15px] items-center'>
                            <div className='w-[40px] h-[40px] bg p-[7px] bg-slate-300 rounded-full'>
                                <img src={stockD.img} alt="" className='w-[40px]' />
                            </div>
                            <div>
                                <h1 className='font-bold text-xl'>{stockD.symbol}</h1>
                                <p className='text-xs text-gray-500'>{stockD.name}</p>
                            </div>
                        ...
                        <div>
                            <h1 className='font-bold'>GHC{price}</h1>
                            <p className={`font-semibold ${priceChangePercentage <=0 ? 'text-red-600' : 'text-green-600'}`}>{priceChangePercentage}% </p>
                        </div>
                ...
                <div className=' rounded-lg bg-white py-[15px] px-[20px] w-[280px] h-[425px] mr-[30px] mt-[100px] shadow-lg'>
                    ...
                    <div className='px-[10px] py-[20px] mt-[20px] rounded-lg'>
                        ...
                        <div className='mt-[5px] w-[220px] bg-gray-200 p-[5px] rounded-lg '>
                            <input type="number" required={true} className='w-full bg-transparent outline-none border-none' onChange={handleQuantityValue} value={quantityValue} placeholder='Quantity' />
                        </div>
                        ...
                        <div className='mt-[5px] w-[220px] bg-gray-200 p-[5px] rounded-lg '>
                            <input type="number" required={true} className='w-full bg-transparent outline-none border-none' onChange={handlePriceValue} value={priceValue} placeholder='Price' />
                        </div>
